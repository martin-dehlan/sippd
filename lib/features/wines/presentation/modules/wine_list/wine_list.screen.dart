...
class _WineListScreenState extends ConsumerState<WineListScreen>
  ...
  // Long-press a tile to enter delete mode: every tile reveals a × badge
  // and taps stop navigating so a stray tap can't open a wine mid-delete.
  ...
  Future<void> _confirmDelete(WineEntity wine) async {
    ...
    if (confirmed == true) {
      await ref.read(wineControllerProvider.notifier).deleteWine(wine.id);
      _exitDeleteMode();
    }
  }
                      ...
                      children: [
                        ...
                        if (_deleteMode)
                          _HeaderIconButton(
                            ...
                            tooltip: l10n.winesListDeleteDone,
                          )
                        else ...[
                          _HeaderIconButton(
                            icon: PhosphorIconsRegular.scales,
                            onTap: () => startCompareSelection(context),
                            tooltip: l10n.winesListTooltipCompare,
                          ),
                          SizedBox(width: context.w * 0.01),
                          _HeaderIconButton(
