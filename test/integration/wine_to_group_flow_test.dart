...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final ownerEntity = (await db.winesDao.getWineById('personal-1'))!.let(
        (row) => WineEntity(
          id: row.id,
          name: row.name,
          rating: row.rating,
      ...
      expect(
        merged.firstWhere((r) => r.userId == ownerId).rating,
        9.5,
  ...
  test('idempotent: owner duplicating their rating into group_wine_ratings '
  ...
  test('delete wine drops the local row and forwards to Supabase', () async {
    final wine = WineEntity(
      id: 'personal-1',
      name: 'X',
      rating: 7,
